...
// We want to be able to set and remove the `data-blackui-mouse` attribute on the `html` element.
if (typeof window !== 'undefined' && typeof document !== 'undefined') {
  document.addEventListener(
    ...
    (event) => {
      ...
      document.documentElement.dataset.blackuiFocusVisible = ''
    },
    true
  )
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (event.detail === ActivationMethod.Mouse) {
        delete document.documentElement.dataset.blackuiFocusVisible
      }
