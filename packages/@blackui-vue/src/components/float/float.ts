...
export const FloatCursor = {
  ...
  emits: ['show', 'hide', 'update'],
  setup({ globalHideCursor, ...props }: FloatCursorProps, {
    emit,
    slots,
    attrs,
  }: SetupContext<['show', 'hide', 'update']>) {
    function onInitial({ show, reference, floating }: FloatVirtualInitialProps) {
      function open() {
        show.value = true
      }
      ...
      watchEffect(onInvalidate => {
        if (globalHideCursor &&
          !ownerDocument.getElementById('blackui-float-cursor-style')
        ) {
          const style = ownerDocument.createElement('style')
          const head = ownerDocument.head || ownerDocument.getElementsByTagName('head')[0]
          head.appendChild(style)
          style.id = 'blackui-float-cursor-style'
          style.appendChild(ownerDocument.createTextNode([
            '*, *::before, *::after {',
            '  cursor: none !important;',
            '}',
            '.blackui-float-cursor-root {',
            '  pointer-events: none !important;',
            '}',
          ].join('\n')))

          onInvalidate(() => ownerDocument.getElementById('blackui-float-cursor-style')?.remove())
        }
      }, { flush: 'post' })
    ...
    return () => {
      ...
      return h(FloatVirtual, {
        ...
        ...attrs,
        portal: true,
        class: 'blackui-float-cursor-root',
        onInitial,
        onShow: () => emit('show'),
        onHide: () => emit('hide'),
        onUpdate: () => emit('update'),
