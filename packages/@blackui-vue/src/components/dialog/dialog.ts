...
export let Dialog = defineComponent({
  ...
  props: {
    ...
    id: { type: String, default: () => `blackui-dialog-${useId()}` },
  },
  ...
  setup(props, { emit, attrs, slots, expose }) {
    ...
    let resolveRootOfMainTreeNode = computed(() => {
      return (Array.from(ownerDocument.value?.querySelectorAll('body > *') ?? []).find((root) => {
        ...
        if (root.id === 'blackui-portal-root') return false
    ...
    let resolveRootOfParentDialog = computed(() => {
      return (Array.from(ownerDocument.value?.querySelectorAll('[data-blackui-portal]') ?? []).find(
        (root) => root.contains(dom(mainTreeNode)) && root instanceof HTMLElement
      ) ?? null) as HTMLElement | null
    })
    ...
    function resolveRootContainers() {
      ...
      let rootContainers = Array.from(
        ownerDocument.value?.querySelectorAll('html > *, body > *, [data-blackui-portal]') ?? []
      ).filter((container) => {
                    ...
                    ? match(position.value, {
                        ...
                        leaf:
                          ...
                          (props.initialFocus
                            ...
                            : ~FocusTrap.features.FocusLock & ~FocusTrap.features.InitialFocus),
                      })
...
export let DialogOverlay = defineComponent({
  ...
  props: {
    as: { type: [Object, String], default: 'div' },
    id: { type: String, default: () => `blackui-dialog-overlay-${useId()}` },
  },
...
export let DialogBackdrop = defineComponent({
  ...
  props: {
    as: { type: [Object, String], default: 'div' },
    id: { type: String, default: () => `blackui-dialog-backdrop-${useId()}` },
  },
  inheritAttrs: false,
...
export let DialogPanel = defineComponent({
  ...
  props: {
    as: { type: [Object, String], default: 'div' },
    id: { type: String, default: () => `blackui-dialog-panel-${useId()}` },
  },
  setup(props, { attrs, slots, expose }) {
...
export let DialogTitle = defineComponent({
  ...
  props: {
    ...
    id: { type: String, default: () => `blackui-dialog-title-${useId()}` },
  },
  setup(props, { attrs, slots }) {
