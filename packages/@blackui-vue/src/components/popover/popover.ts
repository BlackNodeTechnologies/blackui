...
export let PopoverButton = defineComponent({
  ...
  props: {
    ...
    id: { type: String, default: () => `blackui-popover-button-${useId()}` },
  },
  inheritAttrs: false,
  setup(props, { attrs, slots, expose }) {
    ...
    let sentinelId = `blackui-focus-sentinel-${useId()}`
    ...
    function handleFocus() {
      ...
      function run() {
        ...
        if (result === FocusResult.Error) {
          focusIn(
            getFocusableElements().filter((el) => el.dataset.blackuiFocusGuard !== 'true'),
...
export let PopoverOverlay = defineComponent({
  ...
  setup(props, { attrs, slots }) {
    ...
    let id = `blackui-popover-overlay-${useId()}`
...
export let PopoverPanel = defineComponent({
  ...
  props: {
    ...
    id: { type: String, default: () => `blackui-popover-panel-${useId()}` },
  },
  inheritAttrs: false,
  setup(props, { attrs, slots, expose }) {
    let { focus } = props
    ...
    let beforePanelSentinelId = `blackui-focus-sentinel-before-${useId()}`
    let afterPanelSentinelId = `blackui-focus-sentinel-after-${useId()}`
        ...
        match(direction.value, {
          [TabDirection.Forwards]: () => {
            ...
            for (let element of combined.slice()) {
              if (element.dataset.blackuiFocusGuard === 'true' || panel?.contains(element)) {
