...
it('should be possible to use useLabels and a single Label, and have them linked', async () => {
  ...
  expect(format(container.firstElementChild)).toEqual(
    format(html`
      <div>
        <div aria-labelledby="blackui-label-1">
          <label id="blackui-label-1">I am a label</label>
          <span>Contents</span>
        </div>
      </div>
    `)
...
it('should be possible to use useLabels and multiple Label components, and have them linked', async () => {
  ...
  expect(format(container.firstElementChild)).toEqual(
    format(html`
      <div>
        <div aria-labelledby="blackui-label-1 blackui-label-2">
          <label id="blackui-label-1">I am a label</label>
          <span>Contents</span>
          <label id="blackui-label-2">I am also a label</label>
        </div>
      </div>
    `)
  )
...
it('should be possible to update a prop from the parent and it should reflect in the Label component', async () => {
  ...
  expect(format(container.firstElementChild)).toEqual(
    format(html`
      <div>
        <div aria-labelledby="blackui-label-1">
          <label data-count="0" id="blackui-label-1">I am a label</label>
          <button>+1</button>
        </div>
      </div>
    `)
  ...
  expect(format(container.firstElementChild)).toEqual(
    format(html`
      <div>
        <div aria-labelledby="blackui-label-1">
          <label data-count="1" id="blackui-label-1">I am a label</label>
          <button>+1</button>
        </div>
      </div>
    `)
