...
function getPortalRoot() {
  return document.getElementById('blackui-portal-root')!
}
...
it('should be possible to tamper with the modal root and restore correctly', async () => {
  ...
  // Tamper tamper
  document.body.removeChild(document.getElementById('blackui-portal-root')!)
...
it('should be possible to force the Portal into a specific element using PortalGroup', async () => {
  ...
  expect(document.body.innerHTML).toMatchInlineSnapshot(
    `"<div><div><div data-v-app=\\"\\"><main><aside id=\\"group-1\\">A<div data-blackui-portal=\\"\\">Next to A</div></aside><section id=\\"group-2\\"><span>B</span></section><!--teleport start--><!--teleport end--></main></div></div></div>"`
  )
})
