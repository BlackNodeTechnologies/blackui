...
function getPortalRoot(contextElement?: Element | null) {
  ...
  let existingRoot = ownerDocument.getElementById('blackui-portal-root')
  if (existingRoot) return existingRoot

  let root = ownerDocument.createElement('div')
  root.setAttribute('id', 'blackui-portal-root')
...
export let Portal = defineComponent({
  ...
  setup(props, { slots, attrs }) {
    ...
    onUnmounted(() => {
      let root = ownerDocument.value?.getElementById('blackui-portal-root')
      if (!root) return
